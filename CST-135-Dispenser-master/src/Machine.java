 ...
 * Zachary Swoveland
 *
 * This is our main vendine machine GUI.  It is rigged with other classes such as the dispenser/product
 */
...
public class Machine extends Application{
	...
	// controls how much money the customer is using.
	...
	public HBox header() {//name of owner of vending machine or title of scene
		HBox top = new HBox();
		Label label = new Label("Welcome to the Vending Machine");
	...
	public VBox right() {//holds the right pane that holds selection() and other buttons to help with purchase
		...
		VBox changeDisplay = new VBox();
		changeDisplay.setSpacing(25);
		changeDisplay.setPadding(new Insets(50, 10, 10, 0));
		info.setPromptText("Welcome to the VendCO!");
		
		ok.setOnAction((event) -> {
			if (choiceMade) {
				if(Dispenser.productGrid[letterSelected][numberSelected].getQuantity() > 0 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice() ) {
					...
					// set target node - by moving the imageviews, we can make this more elegant.
					if(letterSelected==0){
						if(numberSelected==0 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(cheeto);
							if(Dispenser.productGrid[letterSelected][numberSelected].getQuantity() >= 1){
								// placeholder, add the image again based on inventory.
							} else {
								// placeholder, can add a sorry/we're out notice
							}
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
							
							
						}
						if(numberSelected==1 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(classicLays);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
							
							
						}
						if(numberSelected==2 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(dorito);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
							
							
						}
						if(numberSelected==3 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(coolRanch);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
							
							
						}
						if(numberSelected==4 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(hotCheeto);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
					...
					if(letterSelected==1){
						if(numberSelected==0){
							transition.setNode(lays);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
							
						}
						if(numberSelected==1 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(reeses);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
						...
						if(numberSelected==3 && Integer.parseInt(money.getText()) >= Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
							transition.setNode(nachoDorito);
							int temp = Integer.parseInt(money.getText()) - Dispenser.productGrid[letterSelected][numberSelected].getPrice();
							change.setText("Your Change: " + Integer.toString(temp));
					...
					//Checks if the quantity is empty and if there is enough funds added for transaction
				} else if (Dispenser.productGrid[letterSelected][numberSelected].getQuantity() == 0 && Integer.parseInt(money.getText()) != Dispenser.productGrid[letterSelected][numberSelected].getPrice()){
					info.setText("Empty and not enough Funds");
					//Checks if there is enough funds added for transaction
				} else if (Dispenser.productGrid[letterSelected][numberSelected].getQuantity() > 0 && Integer.parseInt(money.getText()) != Dispenser.productGrid[letterSelected][numberSelected].getPrice()) {
					info.setText("Not enough Funds");
				}
				choiceMade = false;
			} else {
		...
		change.setMinSize(150, 150);
		change.setStyle("-fx-background-color: black");
		change.setTextFill(Color.WHITE);
		changeDisplay.setAlignment(Pos.CENTER);
		money.setPromptText("Please enter payment in cents");
		changeDisplay.getChildren().addAll(money, change);
		right.getChildren().addAll(info, Selection(), buttons, changeDisplay);
		return right;
	}

	
	
	public HBox Basket() {//holds basket image
		VBox bot = new VBox();
		HBox hold = new HBox();

		bot.getChildren().add(basket);
		hold.getChildren().addAll(bot, exit);
